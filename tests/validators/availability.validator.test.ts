...
describe('validateWeeklyAvailability', () => {
	...
	test('throws for non-object availability', () => {
		expect(() => validateWeeklyAvailability('invalid' as unknown as WeeklyAvailability)).toThrow(
			'Availability must be an object'
		)
		expect(() => validateWeeklyAvailability(123 as unknown as WeeklyAvailability)).toThrow(
			'Availability must be an object'
		)
		expect(() => validateWeeklyAvailability(null as unknown as WeeklyAvailability)).toThrow(
			'Availability must be an object'
		)
	})

	test('throws for missing schedules array', () => {
		expect(() => validateWeeklyAvailability({} as unknown as WeeklyAvailability)).toThrow(
			'Availability.schedules must be an array'
		)
		expect(() => validateWeeklyAvailability({ schedules: 'invalid' } as unknown as WeeklyAvailability)).toThrow(
			'Availability.schedules must be an array'
		)
	})
	...
	test('throws for non-object schedule', () => {
		...
		expect(() => validateWeeklyAvailability(availability as unknown as WeeklyAvailability)).toThrow(
			'Schedule at index 0 must be an object'
		)
	})
	...
	test('throws for missing days array', () => {
		...
		expect(() => validateWeeklyAvailability(availability as unknown as WeeklyAvailability)).toThrow(
			'Schedule at index 0: days must be an array'
		)
	})
	...
	test('throws for invalid day names', () => {
		...
		expect(() => validateWeeklyAvailability(availability as unknown as WeeklyAvailability)).toThrow(
			'Schedule at index 0: invalid day "invalid-day"'
		)
	})
	...
	test('throws for duplicate days in same schedule', () => {
		...
		expect(() => validateWeeklyAvailability(availability as unknown as WeeklyAvailability)).toThrow(
			'Schedule at index 0: duplicate days found'
		)
	})
	...
	test('throws for invalid time format', () => {
		...
		expect(() => validateWeeklyAvailability(availability1 as unknown as WeeklyAvailability)).toThrow(
			'Schedule at index 0: start must be in HH:mm format'
		)
		...
		expect(() => validateWeeklyAvailability(availability2 as unknown as WeeklyAvailability)).toThrow(
			'Schedule at index 0: end must be in HH:mm format'
		)

		const availability3 = {
			schedules: [{ days: ['monday'], start: '25:00', end: '17:00' }],
		}
		expect(() => validateWeeklyAvailability(availability3 as unknown as WeeklyAvailability)).toThrow(
			'Schedule at index 0: start must be in HH:mm format'
		)
	})
	...
	test('throws for invalid time range (start >= end)', () => {
		...
		expect(() => validateWeeklyAvailability(availability2 as unknown as WeeklyAvailability)).toThrow(
			'Schedule at index 0: start time (09:00) must be before end time (09:00)'
		)
	})
	...
	test('throws for overlapping schedules on same day', () => {
		...
		expect(() => validateWeeklyAvailability(availability as unknown as WeeklyAvailability)).toThrow(
			'Overlapping schedules found for monday'
		)
	})
