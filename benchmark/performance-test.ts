...
for (const size of testSizes) {
	...
	const kOverlaps1Time = benchmark('K-overlaps (K=1)', () => {
		const scheduler = new Scheduler(busyTimes)
...
const result = scheduler.findAvailableSlots(
	new Date(0), 
	new Date(timeRange * 4 * 60 * 1000), 
...
for (const size of complexitySizes) {
	const busyTimes = generateBusyTimes(size, timeRange)
	const time = benchmark(`Size ${size}`, () => {
		const intervals = busyTimesToIntervals(busyTimes)
		return findFreeIntervals(intervals, 0, {
			start: 0,
...
for (let i = 1; i < complexityTimes.length; i++) {
	...
	console.log(`${complexitySizes[i-1]} → ${complexitySizes[i]}: time ratio ${timeRatio.toFixed(2)}, expected ~${expectedRatio.toFixed(2)}`)
}
